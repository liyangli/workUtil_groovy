 ...
 * Time: 10:43
 */
public enum StringDemoCache {
    CACHE;

    private Date dateTime =  new Date(1454284800000L);//指定的日期，从那天开始计算
    private int  num = 1;//执行间隔天数
    ...
    private BlockingQueue<String> sqlQueue = new ArrayBlockingQueue<String>(200000); //多线程处理时候用,生产者2个线程
    /**
     * 文件需要追加内容
     * @param data 需要追加的数据
     * @param fileName 文件名称
     */
    public void append(String data,String fileName) throws Exception{
        //内容进行过滤
...
//                System.out.println(insertStart);
            ...
            if(insertStart.startsWith("insert")  && insertStart.endsWith(";")){
                //组装完成后，需要设定并且清空
                makeSQL(insertStart,list);
                System.out.println(insertStart);
//                list.add(line);
...
//        for(String str:list){
//            if(str.endsWith(";") && str.startsWith("insert")){
//                sqlQueue.put(str);
//            }
//        }
        System.out.println("#############################################################");
...
//                list.add(dateSql);
                if(dateSql.endsWith(";") && dateSql.startsWith("insert")){
